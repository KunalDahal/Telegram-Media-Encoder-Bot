...
from src.core.dc_checker import pick_download_client
...
class Worker:
    def __init__(self, task_queue, user_settings_getter, ffmpeg, client, config,
        ...
        self.ffmpeg              = ffmpeg
        self.client              = client          # main bot (DC4) — encode + upload
        self.dc5_client          = dc5_client      # secondary bot (DC5) — download only
        self.config              = config
        ...
        while self.running:
            ...
            try:
                ...
                await self._notify_user(
                    task["user_id"],
                    f"⚠️ Task `{task['task_id'][:8]}` was cancelled.",
                )
    ...
    async def _run_task(self, task: dict):
        ...
        for idx, job in enumerate(jobs, start=1):
            ...
            # Don't delete the file if it's the original download (rename passthrough)
            if os.path.exists(encoded_path) and os.path.abspath(encoded_path) != os.path.abspath(downloaded_path):
    ...
    def _get_download_client(self, task: dict):
        """
        Return the appropriate Pyrogram client for downloading this task's file.
        Files on DC5 are downloaded by the secondary bot; everything else by main.
        """
        file_id = task.get("file_id", "")
        return pick_download_client(file_id, self.client, self.dc5_client)

    async def _download(self, task: dict) -> str | None:
        task_id = task["task_id"]

        # ── Case 1: prefetch already finished ────────────────────────────────
        ...
        # ── Case 2: prefetch still in progress ───────────────────────────────
        ...
        dl_client = self._get_download_client(task)
        downloader = Downloader(self.temp_base, self.task_queue, task_id)
        path = await downloader.download(client=dl_client, task_data=task)
    ...
    # ── Encode (always main bot / ffmpeg) ─────────────────────────────────────

    async def _encode(self, task: dict, input_path: str, job: dict) -> str | None:
        task_id        = task["task_id"]
        processing_mode = job.get("processing_mode", "encode")

        # ── Rename / passthrough: no ffmpeg needed ────────────────────────────
        if processing_mode == "rename":
            # Just return the downloaded file; uploader will rename it on send.
            self.task_queue.update_status(task_id, "encoding", 100)
    ...
    async def _upload(self, task: dict, file_path: str, job: dict):
        ...
        # Upload always goes through the MAIN bot (self.client)
        uploader = Uploader(
            self.client,
            upload_data,
            self.task_queue,
    ...
    async def _resolve_thumbnail(self, task: dict, job: dict) -> str | None:
        ...
        if source_thumb_id:
            ...
            try:
                # Thumbnail download is fine through main client
